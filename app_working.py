...
from utils import ImageProcessor, ModelManager, UIComponents
...
class LithographyHotspotApp:
    ...
    def _load_models(_self):
        ...
        try:
            ...
            return cyclegan, classifier, gradcam
    ...
    def render_sidebar(self) -> Dict[str, Any]:
        ...
        nav_option = st.sidebar.radio(
            ...
            ["🔬 Main App", "📋 About & Info"],
            index=0,
            help="Navigate between application pages"
        )
        
        # Handle navigation
        if nav_option == "📋 About & Info":
            # Import and show about page
            try:
                from pages.about import show_about_page
                show_about_page()
                return {}  # Return empty config to skip main app rendering
            except ImportError:
                st.sidebar.error("❌ About page not found")
    ...
    def process_single_image(self, image: Image.Image, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            cyclegan, classifier, gradcam = self._load_models()
            ...
            if config['input_domain'] == 'Synthetic' and cyclegan:
                with st.spinner("🔄 Applying domain adaptation..."):
                    translated_image = cyclegan.translate(img_array)
            ...
            if config['enable_gradcam'] and gradcam:
                with st.spinner("🎨 Generating explanation..."):
                    gradcam_heatmap = gradcam.generate_heatmap(
                        processed_img,
                        model_name=config['model_name']
                    )
    ...
    def _render_download_options(self, results: List[Dict[str, Any]]):
        ...
        with col1:
            if st.button("📥 Download Translated Images", use_container_width=True):
                st.success("✅ Feature ready - implement download logic!")
    ...
    def render_model_info(self, config: Dict[str, Any]):
        ...
        with col1:
            ...
            tpr = 1 - (1 - fpr) ** (1/model_perf['roc_auc'])
    ...
    def run(self):
        ...
        try:
            ...
            # Only render main app if config is not empty (navigation not redirected)
            if config:
                # Main content
